...
public class BreenBot extends PircBot
{
	...
	/*
	 * This method returns the data of the most recently tweeted message from any
	 * user(parameter).
	 * 
	...
	String getTwitterDataByUsername(String username)
	{
		...
		try
		{
			...
			cf.setDebugEnabled(true).setOAuthConsumerKey("weIgCUu5dV7DiOauHGq3TcQ87")
	...
	/*
	 * This method uses the Alphavantage API to read JSON stock price data from a
	 * server. Returns a string that contains information regarding the stock symbol
	 * argument. Gets most recent closing price (market runs Monday-Friday)
	 * 
	 * Data: Opening price Closing price High price Low price
	 * 
	 */
	...
	/*
	 * This method gets the raw weather data of the cities by city name current
	 * overall condition, maximum temperature, minimum temperature, and current
	 * temperature.
	 * 
	 * The data is returned as a String array of size 4. index 0 = overall condition
	 * index 1 = max temp index 2 = min temp index 3 = current temp
	 * 
	...
	/*
	 * This method gets the raw weather data of the cities by zipcode current
	 * overall condition, maximum temperature, minimum temperature, and current
	 * temperature.
	 * 
	 * The data is returned as a String array of size 4. index 0 = overall condition
	...
	/*
	 * This method calls the open-notify API which returns the current latitude and
	...
	/*
	 * This method calls the funtranslations api and converts an english string
	 * argument to the Dothraki language from the show "Game of Thrones".
	 * 
	...
	/*
	 * This method requests a name of a pokemon from the pokeapi and returns the
	 * name of a pokemon based on its assigned number.
	 */
	...
	/*
	 * Sends a list of commands to help the user understand what can be accomplished
	 * using this bot.
	 */
	...
	/*
	 * Gets the initial command from the user: instructs onMessage how to react and
	 * compute output.
	 */
	 ...
	 * 
	 * Miscellaneous: !repeat !changenick !help
	 */
		...
		try
		{
			if (this.enabled == true)
			{
				if (message.charAt(0) == '!')
				{
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!stockdata"))
					{
					...
					else if (getPrefixCommand(message).contains("weather"))
					{
						if (getPrefixCommand(message).equalsIgnoreCase("!weathercity"))
						{
							...
							weatherDataString = "The weather in " + cityName + " is " + data[0]
									+ " and the max temperature is " + kelvinToFahrenheit(Double.parseDouble(data[1]))
									+ "F while the minimum temperature is "
									+ kelvinToFahrenheit(Double.parseDouble(data[2]))
									+ "F and the current temperature is "
					...
					else if (getPrefixCommand(message).contains("representative"))
					{
						handleWordMessage(channel, sender, message, args);
					}
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!distance"))
					{
						double distance;
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!cprice"))
					{
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!exchange"))
					{
						String symbol = args[0];
						symbol = symbol.toUpperCase();

						sendMessageAndAppend(channel,
								"Exchange rates for " + symbol + " are: " + getExchangeRates(symbol));
					}

					// Gets current position of ISS
					// Format: !iss
					else if (getPrefixCommand(message).equalsIgnoreCase("!iss"))
					{
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!pokefind"))
					{
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!dothraki"))
					{
						String englishText = "";
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!ex"))
					{
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!multiply"))
					{
						double product = 1;
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!factorial"))
					{
					...
					/// ***
					// Gets current time
					else if (getPrefixCommand(message).equalsIgnoreCase("!time"))
					{
					...
					// Quick ping command
					else if (getPrefixCommand(message).equalsIgnoreCase("!ping"))
					{
						String pong = "Pong!";
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!repeat"))
					{
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!changenick"))
					{
						String newUser = "";
					...
					else if (getPrefixCommand(message).equalsIgnoreCase("!help"))
					{
						sendHelpOperations();
					}
	...
	/*
	 * Method handles word messages instead of explicit commands from the user.
	 */
	...
	/*
	 * This method removes a special set of special characters from the string
